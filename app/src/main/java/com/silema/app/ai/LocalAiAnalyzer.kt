...
class LocalAiAnalyzer : AiAnalyzer {
    ...
    override suspend fun explainRisk(records: List<VitalRecord>): Result<String> {
        ...
        return Result.success(
            if (assessment.alerts.isEmpty())
                ...
                "检测到 ${assessment.alerts.size} 项需要关注的信号，最高级别为「${assessment.level.label}」。${assessment.alerts.first().why} 数据仅供健康参考，不构成医疗诊断；紧急情况请拨打 120。"
        )
    }
    ...
    private fun levelToScore(level: RiskLevel): Float = when (level) {
    ...
    private fun buildSummary(assessment: Assessment, total: Int): String {
        ...
        val extra = if (assessment.alerts.isNotEmpty())
            "本次分析覆盖 $total 条记录，识别出 ${assessment.alerts.size} 项关注点。"
        else "本次分析覆盖 $total 条记录。"
        return base + extra
    }
     ...
     */
    private fun buildFindings(records: List<VitalRecord>, assessment: Assessment): List<Finding> {
        ...
        val coreTypes = listOf(
            VitalType.HEART_RATE, VitalType.SPO2,
            VitalType.SYSTOLIC, VitalType.DIASTOLIC, VitalType.TEMPERATURE
        )
        for (type in coreTypes) {
            ...
            val covered = assessment.alerts.any { it.metric.contains(base) || (base == "血压" && it.metric.contains("压")) }
            if (covered) continue
            out += Finding(
    ...
    private fun baseName(type: VitalType): String = when (type) {
    ...
    private fun formatValue(type: VitalType, v: Double): String =
        if (type == VitalType.TEMPERATURE) String.format("%.1f", v) + type.unit
        else v.toLong().toString() + type.unit
}
