...
object Reminders {
    ...
    fun syncMeasurement(context: Context) {
        ...
        if (repo(context).measureReminderOn) {
            ...
            val request =
                PeriodicWorkRequestBuilder<MeasureWorker>(1, TimeUnit.DAYS)
    ...
    fun syncSedentary(context: Context) {
        ...
        if (repo(context).sedentaryReminderOn) {
            val request =
                PeriodicWorkRequestBuilder<SedentaryWorker>(1, TimeUnit.HOURS)
    ...
    class MeasureWorker(
        ...
        override suspend fun doWork(): Result {
            ...
            val bp =
                listOf(VitalType.SYSTOLIC, VitalType.DIASTOLIC)
                    .any { t -> records.any { it.typeId == t.id && it.timestampMillis >= todayStart } }
            if (!bp) missing.add("血压")
            if (records.none { it.typeId == VitalType.HEART_RATE.id && it.timestampMillis >= todayStart }) {
                missing.add(
                    "心率",
                )
            }
            ...
            if (missing.isNotEmpty() && canNotify(applicationContext)) {
                ...
                val notif =
                    NotificationCompat
                        .Builder(applicationContext, CHANNEL_REMINDERS)
