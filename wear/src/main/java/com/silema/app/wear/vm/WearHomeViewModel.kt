...
class WearHomeViewModel @Inject constructor() : ViewModel() {
     ...
     * 体征记录列表（按时间倒序）。
     */
    val records: StateFlow<List<VitalRecord>> = WearStore.records
        .stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), emptyList())

    /**
     * 运动记录列表。
     */
    val workouts: StateFlow<List<Workout>> = WearStore.workouts
        .stateIn(viewModelScope, SharingStarted.WhileSubscribed(5000), emptyList())

    /**
     * 当前风险评估结果（基于最近一次各类型体征）。
     */
    val assessment = WearStore.records
        .map { records ->
            if (records.isEmpty()) {
     ...
     * 当前心率（最新一条）。
     */
    val currentHeartRate: StateFlow<Double?> = WearStore.records
        .map { records ->
            records
     ...
     * 当前血氧（最新一条）。
     */
    val currentSpo2: StateFlow<Double?> = WearStore.records
        .map { records ->
            records
