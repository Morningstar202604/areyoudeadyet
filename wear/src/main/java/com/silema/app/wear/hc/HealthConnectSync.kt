...
class HealthConnectSync(private val context: Context) {
    ...
    suspend fun isAvailable(): Boolean = withContext(Dispatchers.IO) {
        try {
            val intent = Intent("androidx.health.ACTION_SHOW_PERMISSIONS_INTERFACE").apply {
                data = HC_URI
     ...
     */
    suspend fun requestPermissions(): Intent? = withContext(Dispatchers.IO) {
        try {
     ...
     */
    suspend fun syncHeartRate(): List<VitalRecord> = withContext(Dispatchers.IO) {
     ...
     * 同步步数数据
     */
    suspend fun syncSteps(): List<VitalRecord> = withContext(Dispatchers.IO) {
        val records = mutableListOf<VitalRecord>()
        try {
     ...
     * 同步血氧数据
     */
    suspend fun syncOxygenSaturation(): List<VitalRecord> = withContext(Dispatchers.IO) {
        val records = mutableListOf<VitalRecord>()
        try {
     ...
     * 同步体温数据
     */
    suspend fun syncBodyTemperature(): List<VitalRecord> = withContext(Dispatchers.IO) {
        val records = mutableListOf<VitalRecord>()
        try {
     ...
     * 同步所有健康数据
     */
    suspend fun syncAll(): List<VitalRecord> = withContext(Dispatchers.IO) {
     ...
     */
    suspend fun writeHeartRate(heartRate: Double, timestampMillis: Long): Boolean = withContext(Dispatchers.IO) {
        try {
     ...
     */
    suspend fun writeOxygenSaturation(percentage: Double, timestampMillis: Long): Boolean = withContext(Dispatchers.IO) {
        try {
     ...
     */
    suspend fun writeBodyTemperature(temperature: Double, timestampMillis: Long): Boolean = withContext(Dispatchers.IO) {
        try {
